...
export default function ProductCard({ product, saved = false, onToggleSave, variant = "default", compact = false, onCategoryClick }) {
  ...
  return (
    <div
      ...
      style={{ borderRadius: 16, cursor: "pointer", padding: 2, background: "#e5e7eb", minWidth: compact ? 200 : 220 }}
    >
      <div style={{ background: "#fff", borderRadius: 13, overflow: "hidden", fontFamily: "Inter, sans-serif" }}>
        ...
        <div style={{ padding: "14px 14px 0", cursor: "default" }}>
          ...
          <p style={{ fontSize: 16, fontWeight: 800, color: "#111827", marginBottom: 4, lineHeight: 1.2 }}>{product.name}</p>
          {product.spec && <p style={{ fontSize: 13, color: "#9ca3af", lineHeight: 1.4 }}>{product.spec}</p>}
        </div>
        ...
        {/* HARGA & RATING */}
        <div style={{ padding: "14px 14px 14px", paddingTop: 12, cursor: "default" }}>
