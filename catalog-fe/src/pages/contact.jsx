...
import { MapPin, Phone, Mail, Clock, ChevronRight, X, Building2, ArrowUpRight, ExternalLink } from "lucide-react";
...
const InstagramIcon = ({ size = 20 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <path fill="#ffffff" d="M12 2.163c3.204 0 3.584.012 4.85.07 1.366.062 2.633.336 3.608 1.311.975.975 1.249 2.242 1.311 3.608.058 1.265.07 1.645.07 4.849s-.012 3.584-.07 4.85c-.062 1.366-.336 2.633-1.311 3.608-.975.975-2.242 1.249-3.608 1.311-1.265.058-1.645.07-4.849.07s-3.584-.012-4.85-.07c-1.366-.062-2.633-.336-3.608-1.311-.975-.975-1.249-2.242-1.311-3.608C2.175 15.584 2.163 15.204 2.163 12s.012-3.584.07-4.85c.062-1.366.336-2.633 1.311-3.608.975-.975 2.242-1.249 3.608-1.311C8.416 2.175 8.796 2.163 12 2.163zm0-2.163C8.741 0 8.333.014 7.053.072 5.197.157 3.355.673 1.965 2.063.573 3.453.157 5.195.072 7.053.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.085 1.858.501 3.6 1.893 4.99 1.39 1.39 3.132 1.808 4.99 1.893C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 1.858-.085 3.6-.501 4.99-1.893 1.392-1.39 1.808-3.132 1.893-4.99.058-1.28.072-1.689.072-4.948 0-3.259-.014-3.667-.072-4.947-.085-1.858-.501-3.6-1.893-4.99C20.548.673 18.805.157 16.948.072 15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
  </svg>
);
...
function SocialIcon({ sl, size = 44, iconSize = 22 }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button onClick={() => setOpen((v) => !v)} title={sl.label}
        style={{ width: `${size}px`, height: `${size}px`, borderRadius: "10px", background: sl.bg, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", flexShrink: 0, padding: 0, boxShadow: "0 4px 12px rgba(0,0,0,0.25)", outline: open ? "2.5px solid rgba(255,255,255,0.55)" : "none", outlineOffset: "2px", transition: "transform .15s, box-shadow .15s" }}
      ...
      {open && (
        <div style={{ position: "absolute", bottom: "calc(100% + 10px)", left: "50%", transform: "translateX(-50%)", background: "#fff", borderRadius: "14px", padding: "16px", boxShadow: "0 16px 48px rgba(7,43,80,0.18)", border: "1px solid #dce6f0", width: "200px", zIndex: 99, animation: "popIn .18s cubic-bezier(.34,1.56,.64,1) both" }}>
          <div style={{ position: "absolute", bottom: "-6px", left: "50%", transform: "translateX(-50%) rotate(45deg)", width: "12px", height: "12px", background: "#fff", border: "1px solid #dce6f0", borderTop: "none", borderLeft: "none" }} />
          <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
            <div style={{ width: "28px", height: "28px", borderRadius: "7px", background: sl.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{sl.icon(13)}</div>
            <span style={{ fontSize: "13px", fontWeight: 700, color: "#111827" }}>{sl.label}</span>
          </div>
          <p style={{ fontSize: "13px", fontWeight: 700, color: "#072B50", margin: "0 0 3px 0" }}>{sl.username}</p>
          <p style={{ fontSize: "11px", color: "#9ca3af", margin: "0 0 12px 0", lineHeight: 1.45 }}>{sl.desc}</p>
          <a href={sl.href} target="_blank" rel="noopener noreferrer"
            style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "5px", width: "100%", padding: "8px", borderRadius: "8px", background: btnBg, color: "#fff", textDecoration: "none", fontSize: "12px", fontWeight: 700, boxSizing: "border-box" }}>
...
function BranchCard({ branch, index, onClick }) {
  ...
  return (
    <div className="bcard" onClick={onClick} style={{ cursor: "pointer", borderRadius: "18px", overflow: "hidden", background: "#fff", border: `1px solid ${NAVY_BORDER}`, display: "flex", flexDirection: "column", boxShadow: "0 2px 12px rgba(7,43,80,.07)" }}>
      <div style={{ position: "relative", height: "180px", overflow: "hidden", flexShrink: 0 }}>
        {hasImg ? (
          <img src={branch.image} alt={branch.name} onError={() => setImgErr(true)} className="bcard-img" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", transition: "transform .55s cubic-bezier(.25,.46,.45,.94)" }} />
        ) : (
          <div style={{ width: "100%", height: "100%", background: `linear-gradient(145deg, ${NAVY} 0%, ${accent} 100%)`, position: "relative", overflow: "hidden" }}>
            ...
            <span style={{ position: "absolute", right: "-8px", bottom: "-16px", fontSize: "100px", fontWeight: 900, color: "rgba(255,255,255,0.06)", lineHeight: 1, userSelect: "none" }}>{String(index + 1).padStart(2, "0")}</span>
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "10px" }}>
              <div style={{ width: "52px", height: "52px", borderRadius: "14px", border: "1.5px solid rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Building2 size={24} style={{ color: "rgba(255,255,255,0.55)" }} />
              </div>
              <span style={{ fontSize: "10px", letterSpacing: "2.5px", textTransform: "uppercase", color: "rgba(255,255,255,0.35)", fontWeight: 600 }}>Foto Segera Hadir</span>
            </div>
          </div>
        )}
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "80px", background: "linear-gradient(to top, rgba(7,43,80,.7) 0%, transparent 100%)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", top: "14px", left: "14px", display: "flex", alignItems: "center", gap: "5px", background: "rgba(255,255,255,0.95)", borderRadius: "30px", padding: "5px 11px", boxShadow: "0 2px 10px rgba(0,0,0,.18)" }}>
      ...
      <div style={{ padding: "20px 20px 22px", display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "14px" }}>
          <div style={{ width: "20px", height: "2px", borderRadius: "2px", background: accent }} />
          <div style={{ flex: 1, height: "1px", background: NAVY_BORDER }} />
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "9px", flex: 1 }}>
          {[{ icon: <MapPin size={11} />, value: branch.address }, { icon: <Phone size={11} />, value: branch.phone }, { icon: <Clock size={11} />, value: branch.hours }].map((row, j) => (
            <div key={j} style={{ display: "flex", gap: "9px", alignItems: "flex-start" }}>
              <span style={{ color: accent, flexShrink: 0, marginTop: "2px" }}>{row.icon}</span>
              <span style={{ fontSize: "12px", color: "#5a6882", lineHeight: 1.55 }}>{row.value}</span>
            </div>
          ))}
        </div>
        <div className="bcard-footer" style={{ marginTop: "18px", paddingTop: "14px", borderTop: `1px solid ${NAVY_BORDER}`, display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
          <div className="bcard-chevron" style={{ display: "flex", alignItems: "center", gap: "4px", padding: "5px 12px 5px 10px", borderRadius: "20px", background: NAVY_FAINT, border: `1px solid ${NAVY_BORDER}`, transition: "all .25s ease" }}>
            <span style={{ fontSize: "11px", fontWeight: 600, color: NAVY }}>Detail</span>
...
export default function Contact() {
  ...
  return (
    <div style={{ background: "#f4f7fb", minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
      ...
      {/* HERO */}
      <div style={{ background: "#fff", padding: "60px 40px", textAlign: "center", borderBottom: `1px solid ${NAVY_BORDER}` }}>
        <h1 style={{ fontSize: "32px", fontWeight: 800, color: NAVY, margin: "0 0 12px 0", letterSpacing: "-0.5px" }}>Contact us</h1>
        <p style={{ fontSize: "15px", color: "#6b7280", margin: "0 auto", maxWidth: "440px", lineHeight: 1.75 }}>
      ...
      <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "30px 40px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "28px", marginBottom: "36px" }}>
          <div style={{ background: "#fff", borderRadius: "16px", padding: "32px", border: `1px solid ${NAVY_BORDER}` }}>
            <h2 style={{ fontSize: "20px", fontWeight: 700, color: NAVY, margin: "0 0 6px 0" }}>Informasi Kontak</h2>
            <p style={{ fontSize: "13px", color: "#9ca3af", margin: "0 0 28px 0", lineHeight: 1.6 }}>Jangan ragu untuk menghubungi kami kapan saja.</p>
            <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
              {[
                ...
                <div key={i} style={{ display: "flex", gap: "14px", alignItems: "flex-start" }}>
                  <div style={{ width: "38px", height: "38px", borderRadius: "9px", background: NAVY, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", flexShrink: 0 }}>{item.icon}</div>
                  <div>
                    <p style={{ fontSize: "11px", color: "#9ca3af", margin: "0 0 3px 0", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px" }}>{item.label}</p>
                    <p style={{ fontSize: "13px", color: "#111827", margin: 0, fontWeight: 600, lineHeight: 1.5 }}>{item.value}</p>
                  </div>
                </div>
              ))}
            </div>
            <div style={{ marginTop: "24px" }}>
              <div onClick={() => setJamOpen(!jamOpen)} style={{ background: NAVY_FAINT, borderRadius: jamOpen ? "10px 10px 0 0" : "10px", padding: "13px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", border: `1px solid ${jamOpen ? NAVY : NAVY_BORDER}`, borderBottom: jamOpen ? `1px solid ${NAVY_BORDER}` : undefined, transition: "all .2s" }}>
                <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                  <Clock size={15} style={{ color: NAVY }} />
                  <div>
                    <p style={{ fontSize: "11px", color: "#6b7280", margin: 0, fontWeight: 600 }}>Jam Operasional</p>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", marginTop: "2px" }}>
                      <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: isOpen ? "#22c55e" : "#9ca3af", animation: isOpen ? "pulse2 2s infinite" : "none" }} />
                      <p style={{ fontSize: "11px", color: isOpen ? "#15803d" : "#9ca3af", margin: 0, fontWeight: 600 }}>
              ...
              {jamOpen && (
                <div style={{ border: `1px solid ${NAVY}`, borderTop: "none", borderRadius: "0 0 10px 10px", overflow: "hidden", animation: "fadeUp .2s ease both" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
                    <div style={{ borderRight: `1px solid ${NAVY_BORDER}` }}>
                      <p style={{ fontSize: "10px", fontWeight: 700, color: NAVY, opacity: 0.4, letterSpacing: "1.5px", textTransform: "uppercase", margin: 0, padding: "10px 14px 6px", borderBottom: `1px solid ${NAVY_BORDER}` }}>Jam Kerja Pusat</p>
                      {[{ label: "Senin – Jumat", value: "08.30 – 17.00", libur: false }, { label: "Sabtu", value: "08.00 – 15.00", libur: false }, { label: "Minggu & Nasional", value: "Libur", libur: true }].map((row, i, arr) => (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 14px", borderBottom: i < arr.length - 1 ? `1px solid ${NAVY_BORDER}` : "none", background: "#fff" }}>
                          <span style={{ fontSize: "12px", color: row.libur ? "#9ca3af" : "#374151" }}>{row.label}</span>
                          <span style={{ fontSize: "12px", fontWeight: 700, color: row.libur ? "#9ca3af" : NAVY }}>{row.value}</span>
                        </div>
                      ))}
                    </div>
                    <div>
                      <p style={{ fontSize: "10px", fontWeight: 700, color: NAVY, opacity: 0.4, letterSpacing: "1.5px", textTransform: "uppercase", margin: 0, padding: "10px 14px 6px", borderBottom: `1px solid ${NAVY_BORDER}` }}>Jam Shift Cabang</p>
                      {[{ name: "Marina", shift1: "09.30–16.00", shift2: "16.00–21.30" }, { name: "Store Street", shift1: "07.30–14.30", shift2: "15.00–22.00" }].map((row, i, arr) => (
                        <div key={i} style={{ padding: "9px 14px", borderBottom: i < arr.length - 1 ? `1px solid ${NAVY_BORDER}` : "none", background: "#fff" }}>
                          <p style={{ fontSize: "10px", fontWeight: 700, color: NAVY, opacity: 0.5, textTransform: "uppercase", letterSpacing: "0.5px", margin: "0 0 5px 0" }}>{row.name}</p>
                          <div style={{ display: "flex", gap: "5px" }}>
                            {[row.shift1, row.shift2].map((sh, j) => (
                              <div key={j} style={{ flex: 1, padding: "5px 8px", background: NAVY_FAINT, border: `1px solid ${NAVY_BORDER}`, borderRadius: "6px" }}>
                                <p style={{ fontSize: "9px", color: "#9ca3af", margin: "0 0 1px 0", fontWeight: 600 }}>Shift {j + 1}</p>
                                <p style={{ fontSize: "11px", fontWeight: 700, color: NAVY, margin: 0 }}>{sh}</p>
                              </div>
                            ))}
                          </div>
                        </div>
          ...
          <div style={{ borderRadius: "16px", position: "relative", overflow: "hidden", minHeight: "400px", border: `1px solid ${NAVY_BORDER}` }}>
            <img src={ceoImg} alt="CEO" style={{ width: "100%", height: "100%", objectFit: "contain", objectPosition: "top", position: "absolute", top: 0, left: 0 }} />
            <div style={{ position: "absolute", inset: 0, background: `linear-gradient(to top, ${NAVY} 38%, transparent 100%)` }} />
            <div style={{ position: "absolute", bottom: "24px", left: "24px", right: "24px" }}>
              <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.8)", margin: "0 0 14px 0", fontStyle: "italic", lineHeight: 1.8, fontWeight: 300 }}>
                " Kami berkomitmen untuk menghadirkan produk teknologi terbaik dengan pelayanan yang profesional dan terpercaya. "
              </p>
              <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                <div style={{ width: "2px", height: "32px", background: "rgba(255,255,255,0.4)", borderRadius: "2px" }} />
                <div>
                  <p style={{ fontSize: "14px", fontWeight: 700, color: "#fff", margin: "0 0 2px 0" }}>Jeon Jungkook</p>
                  <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)", margin: 0, fontWeight: 500 }}>CEO & Founder PT. Indo Bismar</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: "28px", marginBottom: "36px" }}>
          <div style={{ background: "#fff", borderRadius: "16px", padding: "32px", border: `1px solid ${NAVY_BORDER}` }}>
            <h2 style={{ fontSize: "20px", fontWeight: 700, color: NAVY, margin: "0 0 6px 0" }}>Kirim Pesan</h2>
            <p style={{ fontSize: "13px", color: "#9ca3af", margin: "0 0 24px 0" }}>Isi form di bawah dan kami akan segera menghubungi Anda.</p>
                ...
                {[{ label: "Nama Lengkap", placeholder: "John Doe", type: "text" }, { label: "Email", placeholder: "john@email.com", type: "email" }].map((f) => (
                  <div key={f.label}>
                    ...
                    <input type={f.type} placeholder={f.placeholder}
                      style={{ width: "100%", padding: "10px 12px", borderRadius: "8px", border: `1px solid ${NAVY_BORDER}`, fontSize: "13px", outline: "none", boxSizing: "border-box", color: "#111827" }}
                      onFocus={(e) => (e.currentTarget.style.border = `1px solid ${NAVY}`)}
                      onBlur={(e)  => (e.currentTarget.style.border = `1px solid ${NAVY_BORDER}`)} />
                  </div>
                ))}
              </div>
              <div>
                ...
                <input type="tel" placeholder="+62 812-xxxx-xxxx"
                  style={{ width: "100%", padding: "10px 12px", borderRadius: "8px", border: `1px solid ${NAVY_BORDER}`, fontSize: "13px", outline: "none", boxSizing: "border-box" }}
                  onFocus={(e) => (e.currentTarget.style.border = `1px solid ${NAVY}`)}
                  onBlur={(e)  => (e.currentTarget.style.border = `1px solid ${NAVY_BORDER}`)} />
              </div>
              <div>
                <label style={{ fontSize: "12px", fontWeight: 600, color: "#374151", display: "block", marginBottom: "6px" }}>Pesan</label>
                <textarea placeholder="Tulis pesan Anda di sini..." rows={5}
                  style={{ width: "100%", padding: "10px 12px", borderRadius: "8px", border: `1px solid ${NAVY_BORDER}`, fontSize: "13px", outline: "none", resize: "vertical", boxSizing: "border-box", fontFamily: "inherit" }}
                  onFocus={(e) => (e.currentTarget.style.border = `1px solid ${NAVY}`)}
                  onBlur={(e)  => (e.currentTarget.style.border = `1px solid ${NAVY_BORDER}`)} />
              </div>
              <button style={{ width: "100%", padding: "12px", borderRadius: "9px", background: NAVY, color: "#fff", fontSize: "13px", fontWeight: 700, border: "none", cursor: "pointer" }}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <div style={{ background: "#fff", borderRadius: "16px", padding: "22px 24px", border: `1px solid ${NAVY_BORDER}` }}>
              <h3 style={{ fontSize: "20px", fontWeight: 700, color: NAVY, margin: "0 0 4px 0" }}>Follow Us</h3>
              <p style={{ fontSize: "12px", color: "#9ca3af", margin: "0 0 14px 0" }}>Klik icon untuk melihat info akun</p>
              <div style={{ display: "flex", gap: "10px" }}>
                {socialLinks.map((sl) => <SocialIcon key={sl.label} sl={sl} size={44} iconSize={22} />)}
              </div>
            </div>
            <div style={{ borderRadius: "16px", overflow: "hidden", border: `1px solid ${NAVY_BORDER}`, flex: 1 }}>
              <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3957.4!2d112.7!3d-7.25!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zN8KwMTUnMDAuMCJTIDExMsKwNDInMDAuMCJF!5e0!3m2!1sen!2sid!4v1"
                width="100%" height="100%" style={{ border: 0, display: "block", minHeight: "260px" }} allowFullScreen loading="lazy" />
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: "60px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "28px" }}>
            <div>
              <p style={{ fontSize: "11px", fontWeight: 700, color: NAVY, textTransform: "uppercase", margin: "0 0 7px 0", opacity: 0.4 }}>Jaringan Kami</p>
              <h2 style={{ fontSize: "28px", fontWeight: 800, color: NAVY, margin: 0, letterSpacing: "-0.3px" }}>Cabang Kami</h2>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", border: `1px solid ${NAVY_BORDER}`, borderRadius: "30px", padding: "7px 16px", background: "#fff" }}>
              <div style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#22c55e", boxShadow: "0 0 0 3px rgba(34,197,94,.2)" }} />
              <span style={{ fontSize: "12px", color: "#374151", fontWeight: 600 }}>{branches.length} Cabang Aktif</span>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px" }}>
            {branches.map((branch, i) => <BranchCard key={i} branch={branch} index={i} onClick={() => setSelectedBranch(branch)} />)}
          </div>
        </div>
