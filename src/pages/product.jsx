...
const allProducts = Array.from({ length: 24 }, (_, i) => ({
  ...
  image: [laptopImg, mouseImg, keyboardImg, soundImg, kabelImg, monitorImg][i % 6],
}));

const categories = ["Keyboard", "Kabel Lan", "Laptop", "Speaker", "Mouse", "Handphone", "Komputer (PC)"];
    ...
    <div style={{ marginBottom: "20px" }}>
      <button
        ...
        style={{
          ...
          cursor: "pointer", padding: "0 0 10px 0", outline: "none",
        }}
      >
      ...
      <div style={{ overflow: "hidden", maxHeight: open ? "500px" : "0px", transition: "max-height 0.3s ease" }}>
        {children}
      </div>
    </div>
  );
...
export default function Product() {
  ...
  const filteredProducts = allProducts
    .filter((p) => {
      ...
      if (selectedDiscounts.length > 0) {
        const hasDiskon = selectedDiscounts.includes("Diskon") && p.discount !== null;
        const hasBestSeller = selectedDiscounts.includes("Best Seller") && p.rating >= 4.7;
        const hasNewArrival = selectedDiscounts.includes("New Arrival") && p.badge === "New";
  ...
  const hasActiveFilters = Boolean(
    ...
    selectedDiscounts.length > 0 || selectedStatus.length > 0 || priceRange !== 20000000
  );

  const checkboxLabel = (label, checked, onChange) => (
    <label
      key={label}
      style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px", color: "#374151", marginBottom: "10px", cursor: "pointer" }}
    >
      <div
        onClick={onChange}
        style={{
  ...
  return (
    <div style={{ background: "#f9fafb", minHeight: "100vh" }}>
      ...
      {/* HERO BANNER */}
      <div style={{
        background: "linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('/src/assets/hero1.jpg') center/cover",
        minHeight: "320px",
        position: "relative",
        display: "flex",
        alignItems: "center",
        ...
        <div style={{ position: "absolute", top: "20px", right: "40px" }}>
          <div style={{ display: "flex", alignItems: "center", background: "#D5D7DA", borderRadius: "10px", overflow: "hidden", border: "1px solid #D5D7DA", backdropFilter: "blur(6px)" }}>
            <input
              type="text"
              placeholder="Search"
              value={searchInput}
        ...
        <div>
          <h1 style={{ fontSize: "32px", fontWeight: 700, color: "#fff", margin: "0 0 12px 0" }}>Catalog Product</h1>
      ...
      <div style={{
        ...
        {/* SIDEBAR FILTER */}
        <div style={{ background: "#fff", borderRadius: "12px", padding: "24px", height: "fit-content", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
          <h3 style={{ fontSize: "18px", fontWeight: 700, color: "#111827", margin: "0 0 20px 0" }}>Filter Options</h3>

          <FilterSection title="Category">
            {categories.map((cat) => checkboxLabel(cat, selectedCategories.includes(cat), () => toggleItem(selectedCategories, setSelectedCategories, cat)))}
          </FilterSection>
          <hr style={{ border: "none", borderTop: "1px solid #f3f4f6", margin: "0 0 20px 0" }} />

          <FilterSection title="Brand">
            {brands.map((brand) => checkboxLabel(brand, selectedBrands.includes(brand), () => toggleItem(selectedBrands, setSelectedBrands, brand)))}
          </FilterSection>
          <hr style={{ border: "none", borderTop: "1px solid #f3f4f6", margin: "0 0 20px 0" }} />

          <FilterSection title="Diskon">
            {discounts.map((d) => checkboxLabel(d, selectedDiscounts.includes(d), () => toggleItem(selectedDiscounts, setSelectedDiscounts, d)))}
          </FilterSection>
          <hr style={{ border: "none", borderTop: "1px solid #f3f4f6", margin: "0 0 20px 0" }} />
          ...
          <FilterSection title="Harga">
            ...
            <input
              type="range" min={4000069} max={20000000} value={priceRange}
          ...
          <FilterSection title="Status Produk">
            {["Tersedia", "Tidak Tersedia"].map((status) => checkboxLabel(status, selectedStatus.includes(status), () => toggleItem(selectedStatus, setSelectedStatus, status)))}
          </FilterSection>
        </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            <div>
              <p style={{ fontSize: "14px", color: "#374151", margin: "0 0 4px 0" }}>
                Showing {filteredProducts.length === 0 ? 0 : (currentPage - 1) * productsPerPage + 1}–{Math.min(currentPage * productsPerPage, filteredProducts.length)} of {filteredProducts.length} results
                {searchQuery && <span style={{ color: "#6b7280" }}> untuk &quot;<strong>{searchQuery}</strong>&quot;</span>}
              </p>
              {hasActiveFilters && (
                <span
                  onClick={() => { setSelectedCategories([]); setSelectedBrands([]); setSelectedDiscounts([]); setSelectedStatus([]); setSearchQuery(""); setSearchInput(""); setPriceRange(20000000); setCurrentPage(1); }}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", position: "relative" }}>
              ...
              <div
                ...
                style={{
                  padding: "7px 36px 7px 12px", borderRadius: "8px", border: "1px solid #e5e7eb",
                  fontSize: "13px", color: "#374151", cursor: "pointer", background: "#fff",
                  userSelect: "none", position: "relative", minWidth: "150px",
              ...
              >
                ...
                <span style={{
                  ...
                  transition: "transform 0.2s ease", color: "#6b7280",
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "14px" }}>
              {currentProducts.map((product, index) => (
                <ProductCard
                  ...
                  saved={saved[index]}
                  onCategoryClick={() => { toggleItem(selectedCategories, setSelectedCategories, product.category); setCurrentPage(1); }}
                  compact
